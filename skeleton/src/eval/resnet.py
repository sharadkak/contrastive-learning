...
__all__ = ['get_encoder', 'Classifier', 'EvalEncoder']
...
class ProjectionHead(nn.Module):
	...
	def __init__(self, in_dim, out_dim = 128):
		...
		self.fc1 = nn.Linear(in_dim, in_dim, bias = True)
	...
	def forward(self, x):
		...
		x = self.fc2(self.relu(x.squeeze(-1)))
...
class Classifier(nn.Module):
	...
	def __init__(self, in_channel, dropout, num_class = 1000, hidden_dim = None):
		super(Classifier, self).__init__()

		if hidden_dim:
			self.layer = nn.Sequential(nn.Linear(in_channel, hidden_dim, bias = False),
						nn.BatchNorm1d(hidden_dim),
						nn.ReLU(inplace = True),
						nn.Linear(hidden_dim, num_class, bias = True))
		else:
			self.layer = nn.Sequential(nn.Dropout(p= dropout),
							nn.Linear(in_channel, num_class))

	def forward(self, x):
		# print('shape inside classifier ', x.shape)
		x = x.view(x.size(0), -1)
		return self.layer(x)
