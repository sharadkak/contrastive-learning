...
class Trainer(object):
	...
	def train(self, num_epochs, start_epoch=0):
		...
		try:
			...
			path = pt.join(ROOT, '../../res/plots/eval/', self.state['suffix']) + '/'
			# print('save plots here ', path)
			...
			plt.savefig(path + 'eval_loss.png')
			plt.close(fig)
			...
			plt.savefig(path + 'eval_metric.png')
			plt.close(fig)
	...
	def train_epoch(self):
		...
		# the trained network should be in eval mode and train the classifier
		network = network.eval() or network
		classifier = self.state['classifier']
		classifier = classifier.train() or classifier
		...
		for iteration, mini_batch in self.state['train_iter']:
			...
			for sample in mini_batch:
				...
				if self.state['epoch'] %10 == 0:
					save_image(sample['image'][0], pt.join(ROOT, '../../res/images/eval/'), "image_%d.png"% self.state['epoch'])

				with torch.no_grad():
					features = network(sample)
				
				# the embedding from network should be given to classifer
	...
	def validate_epoch(self, epoch):
		...
		network = network.eval() or network
		classifier = self.state['classifier']
		classifier = classifier.eval() or classifier
