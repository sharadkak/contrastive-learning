...
from utils import NCELoss, AUGMENTATION, pytorch_augmentation
from resnet import *
...
"""
Data augmentation is very crucial to create multi views. Two views of an image are created 
by applying a set of augmentations with random parameters. 

These augmentation parameters are mentioned in utils.py, and following augmentations are used. 
1. RandomReiszedCrop
2. gaussian blur with p = 0.5
3. horizontal flip with p = 0.5
4. color jitter (hue=0.2, saturation=0.8, brightness=0.8, contrast=0.8) with p = 0.8

These data augmentation and their parameters are chosen from the paper
What makes for good views for contrastive learning? and SImCLR
...
EXP_FOLDER = '../../exp/'
log_location = pt.join(EXP_FOLDER, 'imagenet_multi/')
...
def make_loader(file, params, world_size, rank, device, gpu_augmentation = False):
	"""
	Create dataloader for training which uses worker to preprocess the data
	"""

	file = pt.join(ROOT, params['data_dir'], file)
	...
	worker = ContrastiveWorker(
	    ...
	    transforms = pytorch_augmentation()
	)
...
def make_policy(network, params, lr, k):
	...
	# optimizer = LARSWrapper(SGD(network.parameters(), lr = lr, momentum=momentum, weight_decay=wd), params['lars_eta'])
...
def main(_run, _config, params):
	...
	train_loader = make_loader('train_data.msgpack', params, world_size, rank, device)
	val_loader = make_loader('val_data.msgpack', params, world_size, rank, device)
	...
	trainer = Trainer(network, optimizer, loss, None, policy, None,
				...
				snapshot_interval= 10 if is_master else None,
