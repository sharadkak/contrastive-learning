...
from crumpets.logging import JSONLogger, SilentLogger, get_logfilename
from crumpets.logging import make_printer, print
from crumpets.torch.utils import save
from crumpets.torch.metrics import AverageValue, NoopMetric
from crumpets.torch.policy import NoopPolicy
from crumpets.timing import ETATimer

from utils import save_image, average_buffers
...
class Trainer(object):
    ...
    def train(self, num_epochs, start_epoch=0):
        ...
        try:
            ...
            # save loss plots here
            path = pt.join(ROOT, '../../res/plots/imagenet/')
            train_loss = [i['loss'] for i in self.state['train_metric_values']]
            val_loss = [i['loss'] for i in self.state['val_metric_values']]
    ...
    def train_epoch(self):
        ...
        rand = random.randint((self.state['epoch']-1)*n, ((self.state['epoch']-1)*n) + n)
        ...
        for iteration, mini_batch in self.state['train_iter']:
            ...
            for sample in mini_batch:
                ...
                # get two views for each image from dataloader
                batch_1 = sample['image']
                batch_2 = sample['target_image']

                # once every epoch save images for master gpu
                if (self.state['epoch']%10 == 0) and (iteration % rand == 0) and dist.get_rank() == 0:
                    save_image(batch_1[0], pt.join(ROOT, '../../res/images/imagenet/'), 'view_one_%d.png' % rand)
                    save_image(batch_2[0], pt.join(ROOT, '../../res/images/imagenet/'), 'view_two_%d.png' % rand)

                # forward pass using same encoder for both views
                with torch.cuda.amp.autocast():
                    output1 = network(batch_1)
                    output2 = network(batch_2)

                    l = loss(output1, output2)
    ...
    def validate_epoch(self, epoch):
        ...
        # average batch norm buffers across all GPUs before next forward call
        average_buffers(network)
